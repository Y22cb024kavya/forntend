...
class _BuildOutfitScreenState extends State<BuildOutfitScreen> {
  ...
  Widget _buildUnifiedGrid(
    ...
    return AhviUnifiedOutfitGrid(
      items: items.map((item) => AhviUnifiedOutfitGridItem(
        id: item.id,
        name: item.name,
        category: item.cat,
        resolvedImageUrl: item.displayUrl ?? '',
        isLocked: lockedItemIds.contains(item.id),
        isAnchor: item.id == widget.selectedItem.id,
      )).toList(growable: false),
      onItemTap: _selectItem,
      onToggleLock: _toggleItemLock,
    );
  }
  ...
  Widget _historyLayout3(
    ...
    return LayoutBuilder(builder: (_, box) {
      ...
      // Same derivation as the main board's three-item layout: left tile comes
