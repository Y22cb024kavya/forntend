...
void main() {
  group('creation intent - must schedule/intercept', () {
    const creations = [
      ...
      'Set a reminder for Dolo at 8 PM',
      'Add a reminder for Dolo at 8 PM',
      'Create a reminder for Dolo at 8 PM',
    ];
    ...
    test('generic creation commands extract medicine name and time', () {
      for (final text in const [
        'Set a reminder for Dolo at 8 PM',
        'Add a reminder for Dolo at 8 PM',
        'Create a reminder for Dolo at 8 PM',
      ]) {
        final intent = debugExtractMedicineReminderIntent(text);
        expect(intent, isNotNull, reason: text);
        expect(intent!['name'], 'Dolo', reason: text);
        expect(intent['time'], '8 PM', reason: text);
      }
    });
  });
  ...
  group('reminder-information questions - must not create', () {
    const queries = [
      ...
      'Check my reminders',
      'When will you remind me to take Dolo?',
      'What time do you remind me to take Dolo?',
    ];
  ...
  group('advisory reminder questions - must not create', () {
    const advisoryQuestions = [
      'Should I set a medicine reminder for Dolo at 8 PM?',
      'Do I need to create a tablet reminder for Dolo at 8 PM?',
    ];

    for (final text in advisoryQuestions) {
      test('"$text" is not a creation request', () {
        expect(debugIsMedicineReminderCreateRequest(text), isFalse);
        expect(debugExtractMedicineReminderIntent(text), isNull);
      });
    }
